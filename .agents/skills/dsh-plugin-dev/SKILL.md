...
- `src/index.ts` → `lib/index.js`（后端 ESM，依赖保持外部解析）。
- `src/client.tsx` → `lib/client.js`（前端，`window.__ModuleLoader__.load` 包装形态；react、primitives 等运行时依赖 external，由 shell 模块表提供）。**JSX 必须写在 `.tsx` 文件**（`.ts` 不解析 JSX）。
